...
const envAllowedOrigins = (process.env.ALLOWED_ORIGINS || '')
	...
	.filter(Boolean);
const corsOrigins = envAllowedOrigins.length ? envAllowedOrigins : defaultAllowedOrigins;
const corsOptions = {
	origin: corsOrigins,
	credentials: true,
...
function normalizeDocenteCarrerasPayload(rawList) {
	if (!Array.isArray(rawList)) return [];
	const seen = new Set();
	const normalized = [];
	for (let i = 0; i < rawList.length; i += 1) {
		const item = rawList[i];
		const carreraRaw = typeof item === 'string'
			? item
			: item?.id ?? item?.carreraId ?? item?.carrera_id;
		if (carreraRaw == null) continue;
		const carreraId = String(carreraRaw).trim();
		if (!carreraId || seen.has(carreraId)) continue;
		seen.add(carreraId);
		let prioridadRaw = typeof item?.prioridad === 'number' ? item.prioridad : parseInt(item?.prioridad, 10);
		if (!Number.isFinite(prioridadRaw) && item?.priority != null) {
			prioridadRaw = typeof item.priority === 'number' ? item.priority : parseInt(item.priority, 10);
		}
		const prioridadValue = Number.isFinite(prioridadRaw) ? prioridadRaw : (i + 1);
		const prioridad = prioridadValue > 0 ? prioridadValue : 1;
		const activo = item?.activo === false ? false : true;
		normalized.push({ carreraId, prioridad, activo });
	}
	return normalized;
}

async function syncDocenteCarreras(docenteId, rawList) {
	if (!docenteId || !db.pool) return;
	const carreras = normalizeDocenteCarrerasPayload(rawList);
	const client = await db.pool.connect();
	try {
		await client.query('BEGIN');
		await client.query('DELETE FROM docente_carrera WHERE docente_id = $1', [docenteId]);
		for (const entry of carreras) {
			await client.query(
				`INSERT INTO docente_carrera (docente_id, carrera_id, prioridad, activo)
				 VALUES ($1, $2, $3, $4)
				 ON CONFLICT (docente_id, carrera_id)
				 DO UPDATE SET prioridad = EXCLUDED.prioridad, activo = EXCLUDED.activo`,
				[docenteId, entry.carreraId, entry.prioridad, entry.activo]
			);
		}
...
if(dbReady){
		...
		try{
			const { rows } = await db.query(`
				SELECT d.id,
				       ...
				       d.email,
				       d.titulo,
				       d."contratoHoras" AS "contratoHoras",
		       d."ContratoHoraSemanal" AS "ContratoHoraSemanal",
		       d.carreraId AS "carreraId",
		       d.edad AS edad,
		       d.estadoCivil AS "estadoCivil",
		       d.turno AS turno,
		       COALESCE(d.activo, TRUE) AS activo,
			       d."TotalHrsModulos" AS "TotalHrsModulos",
			       d."Hrs Teóricas" AS "Hrs Teóricas",
			       d."Hrs Prácticas" AS "Hrs Prácticas",
			       d."Total hrs Semana" AS "Total hrs Semana",
			       d.created_at,
			       d.updated_at,
			       COALESCE(
				       json_agg(
			 ...
			 GROUP BY d.id, d.rut, d.nombre, d.email, d.titulo, d."contratoHoras", d."ContratoHoraSemanal", d.carreraId, d.edad, d.estadoCivil, d.turno, d.activo, d."TotalHrsModulos", d."Hrs Teóricas", d."Hrs Prácticas", d."Total hrs Semana", d.created_at, d.updated_at
	...
	app.post('/api/docentes', async (req,res)=>{
		...
		try{
			...
			const carrerasPayload = Array.isArray(d.carrerasAsignadas)
				? d.carrerasAsignadas
				: (Array.isArray(d.carreras)
					? d.carreras
					: (Array.isArray(d.docenteCarreras) ? d.docenteCarreras : null));
			if (carrerasPayload) {
				await syncDocenteCarreras(id, carrerasPayload);
			}
			res.json({ok:true,id});
		}catch(err){ handleDbError(res, err); }
	});
		...
		try{
			...
			const carrerasPayload = Array.isArray(d.carrerasAsignadas)
				...
				: (Array.isArray(d.carreras)
					...
					: (Array.isArray(d.docenteCarreras) ? d.docenteCarreras : null));
			if (carrerasPayload) {
				await syncDocenteCarreras(req.params.id, carrerasPayload);
			}
			res.json({ok:true});
		}catch(err){ handleDbError(res, err); }
	});
